...
public class ChatActivity extends AppCompatActivity {
    ...
    /**
     * Join a chatroom selected by the user. This method is executed from the
     * ChatroomListAdapter class This method checks to make sure the chatroom exists
     * before joining.
     * 
     * @param chatroom
     */
    ...
    /**
     * add the current user to the list of users who have joined the chatroom. Users
     * who have joined the chatroom will receive notifications on chatroom activity.
     * They will receive notifications via a cloud functions sending a cloud message
     * to the chatroom ID (Sending via topic FCM)
     * 
     * @param chatroom
     */
    private void addUserToChatroom(Chatroom chatroom) {
        ...
        reference.child(getString(R.string.dbnode_chatrooms)).child(chatroom.getChatroom_id())
                .child(getString(R.string.field_users)).child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                ...
                for (DataSnapshot singleSnapshot : dataSnapshot.getChildren()) {
                    ...
                    try {
                        if (singleSnapshot.exists()) {
                            ...
                            for (DataSnapshot snapshot : singleSnapshot.child(getString(R.string.field_users))
                                    .getChildren()) {
    ...
    private void getUserSecurityLevel() {
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference();
        Query query = reference.child(getString(R.string.dbnode_users)).orderByChild(getString(R.string.field_user_id))
